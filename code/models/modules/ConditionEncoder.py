...
class PCXA(nn.Module):
    def __init__(self, channels, c2=None, factor=32):
        super(PCXA, self).__init__()
        self.groups = factor
...
class ConEncoder1(nn.Module):
    def __init__(self, in_nc, out_nc, nf, nb, gc=32, scale=4, opt=None):
        ...
        self.pcxa = PCXA(channels=nf,c2=None,factor=32)
    ...
    def forward(self, x, get_steps=False):
        ...
        placeholder=torch.zeros_like(x[:,:3,:,:])
        ...
        fea_down2 = self.pcxa(fea_down2)
        fea_down4 = self.pcxa(fea_down4)
        fea_down8 = self.pcxa(fea_down8)
        #fea_down16 = self.pcxa(fea_down16)

        #fea_down16 = F.interpolate(fea_down16, size = (fea_head.shape[2], fea_head.shape[3]), mode='bilinear', align_corners=False)
